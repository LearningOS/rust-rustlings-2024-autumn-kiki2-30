...
mod test_undirected_graph {
    ...
    fn test_add_node() {
        let mut graph = UndirectedGraph::new();
        assert!(graph.add_node("a")); // Should return true, node added
        assert!(!graph.add_node("a")); // Should return false, node already exists
        assert!(graph.contains("a")); // Should return true, node exists
        assert!(!graph.contains("b")); // Should return false, node does not exist
    }
}