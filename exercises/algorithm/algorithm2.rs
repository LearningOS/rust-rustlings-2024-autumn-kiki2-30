...
impl<T> LinkedList<T> {
    ...
    pub fn reverse(&mut self) {
        ...
        while let Some(mut curr_ptr) = current {
            ...
            current = unsafe { (*curr_ptr.as_ptr()).prev };
        }

        // After reversing, update start and end
...
mod tests {
    ...
    fn test_reverse_linked_list_1() {
        ...
        for (i, &value) in reverse_vec.iter().enumerate() {
            assert_eq!(value, *list.get(i as i32).unwrap());
        }
    }
